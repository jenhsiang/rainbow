...
class index extends CI_Controller {
    ...
    public function service($page = 'service')
    {
        ...
        if(!empty($request["rev"]))
        {
            ...
            }else if(($today[0]*365+ $today[1]*30+ $today[2]) == ($revday[0]*365+ $revday[1]*30+ $revday[2]))
            {
                ...
                $rev_hour = explode(":",explode("-",$request["re_time"])[0]) ;
        ...
        $query = $this->db->select('id, cat, name, img, img2, img3, attendant, intro, attendant_intro, fee, special_fee, time, service_time, city, zone, way, address, firm, eva ')
            ->where('id', $get["id"])
            ->get('service');
        $service = $query->result_array();
        $data['service'] = $service;
    ...
    public function reservatelist($page = 'reservatelist')
    {
        ...
        $reqest = $this->input->post();
        $get = $this->input->get();
        $this->db->select('order.id, service.name, service.img, order.re_date, order.re_time, order.fee, service.city, zone, service.way, order.address, order.state ');
        $this->db->from('service');
        $this->db->join('order', 'service.id = order.service_id');
        $this->db->where('order.user_id',$session["uid"]);
        $this->db->order_by("order.id","desc");
        $this->db->where('order.pay_check',1);
        $query = $this->db->get();
        ...
        $data_state=array("0"=>"未完成","1"=>"取消預約","2"=>"完成服務","3"=>"已收款","4"=>"爭議訂單");
        for($i=0; $i<count($data['service']); $i++)
        {
    ...
    public function reservation($page = 'reservation')
    {
        ...
        if(!empty($get["id"]))
        {
            $this->db->select('order.id, service.name, service.img, order.re_date, order.re_time, order.fee, service.city, zone, service.way, service.address,
             service.attendant, service.intro, service.attendant_intro, order.result_b, order.result_c, service.time, order.result_c, firm_info.tel, firm_info.id as f_id, order.state');
            $this->db->from('service');
            $this->db->join('order', 'service.id = order.service_id');
            $this->db->join('firm_info', 'firm_info.id = service.firm');
            $this->db->where('order.id',$get["id"]);
            $query = $this->db->get();
            $data['service'] = $query->result_array();
    ...
    private function eva($order_id)
    {
        ...
        $this->db->from('score');
        $this->db->join('order', 'order.id = score.order_id');
        $this->db->where('order.service_id',$order[0]["service_id"]);
        $query = $this->db->get();
    ...
    public function reservatelist_firm($page = 'reservatelist_firm')
    {
        ...
        $reqest = $this->input->post();
        $get = $this->input->get();
        $this->db->select('order.id, service.name, service.img, order.re_date, order.re_time, order.fee, service.city, zone, service.way, service.address, order.name as user_name, order.tel, order.state ');
        $this->db->from('service');
        $this->db->join('order', 'service.id = order.service_id');
    ...
    public function reservation_firm($page = 'reservation_firm')
    {
        ...
        {
            $this->db->select('order.id, order.user_id, order.service_id ');
            $this->db->from('order');
            $this->db->where('order.id',$request["talk"]);
            $query = $this->db->get();
            $service = $query->result_array();
        ...
        {
            $this->db->select('order.id, service.name, service.img, order.re_date, order.re_time, order.fee, service.city, zone, service.way, service.address,
             service.attendant, service.intro, service.attendant_intro, order.result_b, order.result_c, service.time, order.result_c, order.name as user_name, order.tel, order.state ');
            $this->db->from('service');
            $this->db->join('order', 'service.id = order.service_id');
            $this->db->where('order.id',$get["id"]);
            $query = $this->db->get();
    ...
    public function score_list($page = 'score_list')
    {
        ...
        if(!empty($get["id"]))
        {
            ...
            $this->db->from('score');
            $this->db->join('order', 'order.id = score.order_id');
            $this->db->where('order.service_id',$get["id"]);
            $query = $this->db->get();
            $data['score'] = $query->result_array();
        }
    ...
    public function ask($page = 'ask')
    {
        ...
        if($session["login"] == 1)
        {
            ...
            $this->db->select('message.id, message.service_id, message.content, message.time, message.read, message.user_type');
    ...
    public function wallet($page = 'wallet')
    {
        ...
        $this->db->select('order.id, service.name, service.img, order.re_date, order.re_time, order.fee, service.city, zone, service.way, service.address, order.gold, order.gold2 ');
        $this->db->from('service');
        $this->db->join('order', 'service.id = order.service_id');
        $this->db->where('order.user_id',$session["uid"]);
        $this->db->where('order.pay_check',1);
        ...
        {
            $data["member_list"][$i]["join_time"] = explode(" ",$data["member_list"][$i]["join_time"])[0] ;
        }

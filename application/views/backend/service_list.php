                            ...
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example2">
                                ...
                                if(!empty($service))
                                {
                                    foreach($service as $item)
                                    {
                                        ...
                                        echo '<td>'.$item["id"].'</td>';
                                        echo '<td>'.$item["name"].'<p><a href="'.$base_url.'index.php/service?id='.$item["id"].'" target="_blank">瀏覽</a></p></td>';
                                        echo '<td>'.$item["fee"].'</td>';
                                        echo '<td>'.$item["fav"].'</td>';
                                        echo '<td>'.$item["buy"].'</td>';
                                        echo '<td>'.$item["eva"].'</td>';
                                        if(!$item["ready"])
                                        {
                                            echo "<td><p style='font-weight: bold; color: #ff2832;'>異常</p></td>";
                                        }elseif($item["ready"] == 2)
                                        {
                                            echo "<td><p style='font-weight: bold; color: #ff2832;'>暫停服務中</p></td>";
                                        }else
                                        {
                                            echo "<td><p style='font-weight: bold; color: #ff2832;'>上架</p></td>";
                                        }
                                        echo '<td class="center"> <button style="width: 100%;" name="down" value="'.$item["id"].'">下架</button><button style="width: 100%;" name="del" value="'.$item["id"].'">刪除</button></td>';
